...
server <- function(input, output) {
  ...
  output$tabela2 <- renderDT({
    ...
    if ("Todos" %in% input$municipio2) {
      ...
      pop_shy2 <- subset(pop_rj_filtrado, municipio %in% input$municipio2) 
    ...
    if ("Todos" %in% input$sexo2) {
      ...
      pop_shy2 <- subset(pop_shy2, sexo %in% input$sexo2)  
    }
  ...
  output$tabela3 <- renderDT({
    ...
    #pop_shy3 <- pop_rj_filtrado 
    #} else {
    #pop_shy3 <- subset(pop_rj_filtrado, municipio %in% input$municipio3) 
    ...
    #pop_shy3 <- pop_shy3
    #} else {
    #pop_shy3 <- subset(pop_shy3, sexo %in% input$sexo3)  
    #}
    ...
    datatable(pop_shy3_3) %>% # Diminuindo a casas decimais das colunas
  ...
  output$tabela4 <- renderDT({
    ...
    datatable(pop_shy4_3_1) %>% # Diminuindo a casas decimais das colunas
